...
In `src/model/types.ts`, replace the comment on the `mapSettings` field (lines 47-50, the two-line block ending `mapSettings: MapSettings;`):
...
- Test: `test/enemyTab.spec.ts` (new), `test/advancedTab.spec.ts` (if it exists - otherwise add a focused assertion in the new enemy spec via a mounted AdvancedTab)
...
```bash
git add src/components/AdvancedTab.vue src/components/EnemyTab.vue test/enemyTab.spec.ts
...
First, add these two imports to the TOP of `test/enemyTab.spec.ts` alongside the existing imports (ES imports must be at module top level, and Biome's import sorter will otherwise complain):
...
- If `test/advancedTab.spec.ts` already exists, move the "Advanced no longer shows the checkboxes" assertion there instead of mounting `AdvancedTab` inside `test/enemyTab.spec.ts`.
